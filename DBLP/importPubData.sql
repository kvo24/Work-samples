...
-- -- Book
-- -- get publishers:
-- create table if not exists tempPublisher (k TEXT, publisher TEXT);
-- insert into tempPublisher (select f.k, f.v from Field f join Pub p on p.k = f.k where p.p = 'book' and f.p = 'publisher');
-- -- k is unique in tempPublisher (there are no books with multiple publishers)
-- --
-- -- get isbns:
-- create table if not exists tempISBNs (k TEXT, isbn TEXT);
-- insert into tempISBNs (select f.k, f.v from Field f join Pub p on p.k = f.k where p.p = 'book' and f.p = 'isbn');
-- -- 
-- -- k is NOT unique in tempISBNs since
-- -- dblp=# select count(*) from tempisbns;
-- -- count 
-- -------
-- -- 22718
-- --
-- -- meanwhile,
-- -- dblp=# select count(distinct k) from tempisbns;
-- -- count 
-- -------
-- -- 18135
-- --
-- -- Let's keep only the maximum ISBN value for a given k:
-- delete from tempISBNs 
-- where isbn not in 
--     (select MAX(isbn) from tempISBNs group by k);
-- -- somehow still one duplicate (k = books/daglib/0018972).
-- -- let's remove it:
-- delete from tempISBNs
-- where k = 'books/daglib/0018972' and isbn = '0387515666';
-- --
-- --
-- -- There also appear to be books with a publisher but no isbn (and possibly vice versa).
-- -- Let's keep only books that have a publisher:
-- create table tempBook (k TEXT, publisher TEXT, isbn TEXT);
-- insert into tempBook (select p.k, p.publisher, i.isbn 
--                         from tempPublisher p left outer join tempISBNs i 
--                         on p.k = i.k);